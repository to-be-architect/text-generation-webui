...
### ubuntu更新nvidia驱动版本
更新Ubuntu系统上的NVIDIA驱动程序可以提高显卡性能，增加新功能或修复已知问题。以下是在Ubuntu系统上更新NVIDIA驱动程序的步骤：
打开终端。你可以按下Ctrl + Alt + T组合键来打开终端。
添加NVIDIA的Ubuntu存储库。运行以下命令将其添加到系统中：
```
sudo add-apt-repository ppa:graphics-drivers/pp
```
执行命令后，会提示你输入密码，输入密码并按Enter键。
更新包列表。运行以下命令更新你的包列表：
```
sudo apt-get update
```
查看可用的NVIDIA驱动程序版本。运行以下命令查看可用的NVIDIA驱动程序版本：

```
apt-cache search nvidia-driver
```
执行命令后，将列出所有可用的NVIDIA驱动程序版本。
安装所需的NVIDIA驱动程序版本。运行以下命令安装所需的NVIDIA驱动程序版本：

```
sudo apt-get install nvidia-driver-<version>
```
将“”替换为你想要安装的NVIDIA驱动程序版本号。
安装完成后，重新启动系统。你可以运行以下命令来重新启动系统：

```
sudo reboot
```
检查NVIDIA驱动程序版本。重新启动后，打开终端，运行以下命令来检查NVIDIA驱动程序版本：

```
nvidia-smi
```
如果你看到NVIDIA驱动程序版本号，则表示安装成功。
以上是在Ubuntu系统上更新NVIDIA驱动程序的步骤。请确保在更新驱动程序之前备份重要数据，并在更新完成后重新启动系统。
...
|  0%   37C    P5              34W / 450W |   1574MiB / 24564MiB |     12%      Default |
|                                         |                      |                  N/A |
+-----------------------------------------+----------------------+----------------------+
|   1  NVIDIA GeForce RTX 3090 Ti     Off | 00000000:05:00.0 Off |                  Off |
|  0%   32C    P8              10W / 450W |     14MiB / 24564MiB |      0%      Default |
|                                         |                      |                  N/A |
+-----------------------------------------+----------------------+----------------------+
...
|        ID   ID                                                             Usage      |
|=======================================================================================|
|    0   N/A  N/A      3027      G   /usr/lib/xorg/Xorg                          951MiB |
|    0   N/A  N/A      3319      G   /usr/bin/gnome-shell                        178MiB |
|    0   N/A  N/A      7255      G   ...ures=SpareRendererForSitePerProcess      106MiB |
|    0   N/A  N/A     20761    C+G   ...21829929,1657412198793813951,262144      304MiB |
|    1   N/A  N/A      3027      G   /usr/lib/xorg/Xorg                            4MiB |
+---------------------------------------------------------------------------------------+
